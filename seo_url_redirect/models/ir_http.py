...
def getSuffix(value):
    ...
    use_suffix = website.use_suffix
    if use_suffix:
        suffix = request.env['website.rewrite'].sudo().getUrlSuffix(value)
    return suffix
...
class ModelConverterRewrite(ModelConverter):
    ...
    def to_python(self, value):
        ...
        if field and field in env[self.model]._fields:
            res = request.env[self.model].sudo().search([(field, '=', value)])
            if res:
                record_id = res[0].id
        if not record_id:
        ...
        if not hasattr(request, "jsonrequest") and not hasattr(request, 'rerouting'):
            ...
            if '/web/image' not in req_page and '/web/content' not in req_page:
                try:
                    ...
                    if redirect and useServerRewrites:
                                ...
                                if redirect:
                                    ...
                                    if rewrite_val == 'product.template':
                                        ...
                                        if frontEndLang != defaultLang:
                                            if frontEndLang:
                                                frontEndLang = frontEndLang.split("_")[0]
