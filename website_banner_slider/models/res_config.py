...
class WebsiteBanner(models.TransientModel):
    ...
    website_banner_type = fields.Selection(
        [('default', 'Pagination'), ('gallery', 'Gallery')], string="Banner Type", default="default")
    wk_banner_ids = fields.Many2many(
        string="Website Banners",	comodel_name='website.banner.images')
    lang_flag = fields.Boolean(
        string="Show Banners on the basis of Language", default=False)

    @api.model
    def set_values(self):
        res = super(WebsiteBanner, self).set_values()
        IrDefault = self.env['ir.default'].sudo()
        IrDefault.set('website.banner.config',
                      'website_banner_type', self.website_banner_type)
        IrDefault.set('website.banner.config',
                      'wk_banner_ids', self.wk_banner_ids.ids)
    ...
    def wk_set_demo_banner_config(self):
        ...
        if banner1 and banner2 and banner3 and banner4 and banner5:
            IrDefault.set('website.banner.config', 'wk_banner_ids', [
                          banner1.id, banner2.id, banner3.id, banner4.id, banner5.id])
        IrDefault.set('website.banner.config',
                      'website_banner_type', 'gallery')
