...
class WebsiteBannerImages(models.Model):
    ...
    state = fields.Selection(
        [('draft', 'Draft'), ('published', 'Published')], string="State", default="draft")
...
class Website(models.Model):
    ...
    def get_website_banner_images(self):
        ...
        wk_banner_ids = ir_default.get(
            'website.banner.config', 'wk_banner_ids') or []
        lang_flag = ir_default.get('website.banner.config', 'lang_flag')
        if lang and lang_flag:
            image_ids = self.env['website.banner.images'].sudo().search(
                [('state', '=', 'published'), ('lang.code', '=', lang), ('id', 'in', wk_banner_ids)])
        else:
            image_ids = self.env['website.banner.images'].sudo().search(
                [('state', '=', 'published'), ('id', 'in', wk_banner_ids)])
        if image_ids:
            return image_ids
        return False
    ...
    def get_website_banner_type(self):
        ...
        website_banner_type = ir_values.sudo().get(
            'website.banner.config', 'website_banner_type')
