...
class WkImportCsv(models.Model):
    ...
    _csv_model_selection = lambda self, * \
        args, **kwargs: self._get_csv_import_model(*args, **kwargs)

    name = fields.Char(string="Name", required=True, copy=False,
                       readonly=True, default=lambda self: _('New'))
    csv_model = fields.Selection(
        _csv_model_selection, string="Select Entity", required=True,
        default=lambda self: self._context.get('csv_model', 'product.product'))
    csv_file = fields.Binary('File', required=True,)
    sample_attachment = fields.Many2one(
        comodel_name="ir.attachment", string="Sample Zip File", readonly=True)
    ...
    def import_function(self):
        ...
        for current_record in self:
            ...
            for data in data_lst:
                if not data:
                    raise UserError("Your Csv File is Blank.")
                for item in data:
                    image_name = ''
                    values = item
                    res = False
    ...
    def validate_image(self, values, path):
        ...
        if values.get('image_1920'):
            image_name = values.get('image_1920')
            if image_name.find('/') < 0:
                image = path + 'images/' + values.get('image_1920')
            else:
                image = values.get('image_1920')
            result = self.read_image(image)
            if result.get('status'):
                values['image_1920'] = result.get('encoded_string')
            else:
